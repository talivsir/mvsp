<?php
if (!function_exists('timelineImg')) {
    function timelineImg($relPath) {
        $abs = __DIR__ . '/../' . $relPath;
        return $relPath . (is_file($abs) ? '?v=' . filemtime($abs) : '');
    }
}
?>
<section class="services-section" id="services">
    <div class="container">

        <!-- Section Header -->
        <div class="section-header text-center" data-scroll-reveal>
            <span class="section-eyebrow section-eyebrow--center">White-Glove Care</span>
            <h2 class="section-title">Premium Services</h2>
            <p class="section-subtitle">Beyond storage. We offer a full suite of white-glove services to ensure your vehicle is always ready for your next Maui adventure.</p>
        </div>

        <!-- Services Grid -->
        <div class="services-grid">
            
            <!-- Service: Complete Detailing -->
            <div class="service-card large-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/complete-detailing.jpg'); ?>" alt="Complete Detailing" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                    </div>
                    <h3>Complete Detailing</h3>
                    <p>Interior deep cleaning, exterior polish, and ceramic coating application. Return to a vehicle that feels brand new.</p>
                </div>
            </div>

            <!-- Service: Airport Delivery -->
            <div class="service-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/airport-delivery.jpg'); ?>" alt="Airport Delivery" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>
                    </div>
                    <h3>Airport Delivery</h3>
                    <p>Step off your flight at OGG directly into your waiting vehicle. No shuttles, no lines.</p>
                </div>
            </div>

            <!-- Service: Car Wash -->
            <div class="service-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/car-wash.jpg'); ?>" alt="Car Wash" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/></svg>
                    </div>
                    <h3>Car Wash</h3>
                    <p>Hand wash using pH-balanced soaps and spot-free rinse to protect your clear coat.</p>
                </div>
            </div>

            <!-- Service: Malama Car Wash -->
            <div class="service-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/malama-car-wash.jpg'); ?>" alt="Malama Car Wash" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                    </div>
                    <h3>Malama Car Wash</h3>
                    <p>Eco-friendly, waterless wash designed to protect Maui's fragile marine ecosystem.</p>
                </div>
            </div>

            <!-- Service: Pier Pickup -->
            <div class="service-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/pier-pickup.jpg'); ?>" alt="Pier Pickup" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1zM4 22v-7"/></svg>
                    </div>
                    <h3>Pier Pickup</h3>
                    <p>Arriving by sea? We'll have your vehicle waiting dockside at Kahului Harbor.</p>
                </div>
            </div>

            <!-- Service: Curbside Pickup -->
            <div class="service-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/curbside-pickup.jpg'); ?>" alt="Curbside Pickup" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
                    </div>
                    <h3>Curbside Pickup</h3>
                    <p>Convenient hotel or residence curbside drop-off across the island.</p>
                </div>
            </div>

            <!-- Service: Vehicle Delivery -->
            <div class="service-card large-card" data-scroll-reveal>
                <div class="service-img-wrapper">
                    <img src="<?php echo timelineImg('img/premiumservices/vehicle-delivery.jpg'); ?>" alt="Vehicle Delivery" class="service-img">
                    <div class="service-gradient-overlay"></div>
                </div>
                <div class="service-content">
                    <div class="service-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                    </div>
                    <h3>Vehicle Delivery</h3>
                    <p>Fully insured, flatbed transport for larger or specialty vehicles, or when you simply prefer zero added miles.</p>
                </div>
            </div>

        </div>

        <!-- Section CTA -->
        <div class="services-cta text-center" data-scroll-reveal>
            <a href="#contact" class="btn btn-primary btn-lg btn-glow btn-animated">Schedule a Service</a>
        </div>

    </div>
</section>
